...
export async function GET(request: NextRequest) {
  try {
    ...
    let filteredStaff: StaffMemberType[] = allStaff;
    if (locationId) {
      if (locationId === 'home') {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Try to create in database first using a transaction for consistency
    try {
      const result = await prisma.$transaction(async (tx) => {
        ...
        // Fetch complete staff member with associations
        return await tx.staffMember.findUnique({
          where: { id: staff.id },
          include: {
            user: true,
            locations: {
      ...
      const transformedStaff = {
        ...
        phone: result.phone || '',
        role: result.jobRole || role, // Use the stored jobRole
        locations: result.locations.map(loc => loc.locationId),
        status: result.status === 'ACTIVE' ? 'Active' : result.status === 'INACTIVE' ? 'Inactive' : 'On Leave',
        avatar: result.avatar || result.name.split(' ').map((n: string) => n[0]).join(''),
        color: result.color || 'bg-purple-100 text-purple-800',
        homeService: result.homeService,
        specialties: result.specialties ? (() => {
          try {
            return JSON.parse(result.specialties);
          } catch (e) {
            console.warn('Failed to parse specialties JSON:', result.specialties);
            return [];
          }
        })() : [],
        ...
        dateOfBirth: result.dateOfBirth ? (() => {
          ...
          if (typeof result.dateOfBirth === 'string') {
            // If already in DD-MM-YY format, return as is
            if (/^\d{2}-\d{2}-\d{2}$/.test(result.dateOfBirth)) {
            ...
            // Handle Date object
            const date = result.dateOfBirth.toISOString().split('T')[0] // YYYY-MM-DD
            const [year, month, day] = date.split('-')
            const shortYear = year.slice(-2) // Get last 2 digits for YY format
            return `${day}-${month}-${shortYear}` // Convert to DD-MM-YY for frontend
          }
